                  ...
                  <div class="sm:flex sm:items-start">
                    <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
                      <DialogTitle as="h3" class="text-lg font-semibold leading-6 text-gray-900 dark:text-white mb-4">
                        {{ userGroup ? 'Takım Düzenle' : 'Yeni Takım' }}
                      </DialogTitle>
...
const { createUserTeam, updateUserTeam } = useUserTeams()
...
const handleSubmit = async () => {
  ...
  try {
    if (props.userTeam) {
      await updateUserTeam(props.userTeam.id, formData.value)
      useToast().showSuccess('Takım başarıyla güncellendi')
    } else {
      await createUserTeam(formData.value)
      useToast().showSuccess('Takım başarıyla eklendi')
    }
    emit('saved')
  } catch (error) {
