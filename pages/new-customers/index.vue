  ...
  <div class="min-h-screen">
      ...
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
              {{ t('new_customers.title', 'Yeni Kişiler') }}
            </h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              {{ t('new_customers.subtitle', 'Yeni durumundaki kişileri görüntüleyin ve yönetin') }}
            </p>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          ...
          >
            ...
            {{ t('new_customers.actions.refresh', 'Yenile') }}
          </button>
          <button 
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="flex items-center justify-between">
          <div>
            <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">
              {{ t('new_customers.stats.total', 'Toplam') }}
            </p>
      ...
      <div class="group bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">
              {{ t('new_customers.stats.active', 'Aktif') }}
            </p>
      ...
      <div class="group bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
        ...
        <div class="h-1 w-full bg-gradient-to-r from-rose-500 from-rose-400 rounded-full mt-4 opacity-60"></div>
      </div>
      ...
      <div class="group bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">
              {{ t('new_customers.stats.filtered', 'Filtre Sonucu') }}
            </p>
            <p class="text-2xl font-bold text-cyan-600 dark:text-cyan-400 mt-1">{{ filteredCustomers.length }}</p>
          </div>
          <div class="h-12 w-12 rounded-xl bg-cyan-100 dark:bg-cyan-900/30 flex items-center justify-center group-hover:scale-110 transition-transform">
      ...
      <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700">
        <div class="flex items-center gap-2">
          ...
          <span class="font-medium text-gray-700 dark:text-gray-300">
            {{ t('new_customers.filters.title', 'Filtreler') }}
          </span>
        </div>
        <button 
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div class="lg:col-span-2">
            <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wide">
              {{ t('new_customers.filters.search', 'Ara') }}
            </label>
            <div class="relative">
              ...
              <input 
                ...
                :placeholder="t('new_customers.filters.search_placeholder', 'İsim, email, telefon veya kaynak...')" 
              />
            </div>
          </div>
          ...
          <div>
            <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wide">
              {{ t('new_customers.filters.status', 'Durum') }}
            </label>
            <select 
            ...
            >
              <option value="">{{ t('new_customers.filters.all_statuses', 'Tüm Durumlar') }}</option>
          ...
          <div class="flex items-end">
            ...
            >
              {{ t('new_customers.filters.clear_filters', 'Filtreleri Temizle') }}
            </button>
          </div>
        </div>
      </div>
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center py-16">
      <div class="relative">
        ...
        <div class="absolute top-0 left-0 w-14 h-14 rounded-full border-4 border-transparent border-t-blue-600 animate-spin"></div>
      </div>
      <p class="mt-4 text-sm text-gray-500 dark:text-gray-400">
        {{ t('new_customers.loading', 'Yükleniyor...') }}
      </p>
    </div>
      ...
      <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <span class="font-medium text-gray-700 dark:text-gray-300">
              {{ t('new_customers.table.title', 'Kişi Listesi') }}
            </span>
        ...
        <table class="min-w-full">
          <thead>
            <tr class="bg-gray-50 dark:bg-gray-900/50">
              <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                {{ t('new_customers.table.person', 'Kişi') }}
              </th>
              <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                {{ t('new_customers.table.status', 'Durum') }}
              </th>
              <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                {{ t('new_customers.table.contact', 'İletişim') }}
              </th>
              <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                {{ t('new_customers.table.source', 'Kaynak') }}
              </th>
              <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                {{ t('new_customers.table.assigned', 'Atanan') }}
              </th>
              <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                {{ t('new_customers.table.date', 'Tarih') }}
              </th>
              <th class="px-6 py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                {{ t('new_customers.table.actions', 'İşlemler') }}
              </th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            >
              ...
              <td class="px-6 py-4">
                <span 
                  v-if="customer.source && customer.source !== '-'"
              ...
              <td class="px-6 py-4">
                <div v-if="customer.relevantUser" class="flex items-center gap-2">
                  <img 
                    v-if="customer.relevantUser?.avatar" 
                    :src="path + customer.relevantUser?.avatar" 
                    alt="Avatar"
              ...
              <td colspan="7" class="px-6 py-16 text-center">
                <div class="flex flex-col items-center">
                  ...
                  <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">
                    {{ searchTerm || statusFilter ? t('new_customers.empty.no_results', 'Sonuç bulunamadı') : t('new_customers.empty.no_people', 'Yeni kişi yok') }}
                  </h3>
                  <p class="text-sm text-gray-500 dark:text-gray-400 max-w-sm">
                    {{ searchTerm || statusFilter ? t('new_customers.empty.no_results_message', 'Arama kriterlerinize uygun kişi bulunamadı.') : t('new_customers.empty.no_people_message', 'Henüz yeni durumunda kişi bulunmuyor.') }}
                  </p>
                  <button 
                    v-if="authStore.user?.role !== 'user'" 
                    @click="showCreateModal = true"
                  ...
                  >
                    ...
                    {{ t('new_customers.empty.add_customer', 'Yeni Müşteri Ekle') }}
                  </button>
                </div>
              </td>
            </tr>
      ...
      <div v-if="pagination.totalPages > 1" class="px-6 py-4 border-t border-gray-100 dark:border-gray-700">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <p class="text-sm text-gray-500 dark:text-gray-400">
            {{ tp('new_customers.pagination.showing', { 
          ...
          >
            ...
            >
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                      {{ t('new_customers.actions_modal.title', 'İşlem seçin') }}
                    </p>
                  </div>
                </div>
                <button 
              ...
              <div class="p-4 grid grid-cols-3 gap-3">
                ...
                <button 
                  @click="showHistory(getActiveCustomer)"
                  class="flex flex-col items-center gap-2 p-4 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-all group"
                >
                  <div class="h-12 w-12 rounded-xl bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center group-hover:scale-110 transition-transform">
                  ...
                  <span class="text-xs font-medium text-gray-700 dark:text-gray-300">
                    {{ t('new_customers.actions_modal.history', 'Geçmiş') }}
                  </span>
                </button>
                ...
                <button 
                  @click="showServices(getActiveCustomer)"
                  class="flex flex-col items-center gap-2 p-4 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-all group"
                >
                  <div class="h-12 w-12 rounded-xl bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center group-hover:scale-110 transition-transform">
                  ...
                  <span class="text-xs font-medium text-gray-700 dark:text-gray-300">
                    {{ t('new_customers.actions_modal.services', 'Hizmetler') }}
                  </span>
                </button>

                <button 
                  @click="showFiles(getActiveCustomer)"
                  class="flex flex-col items-center gap-2 p-4 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-all group"
                >
                  <div class="h-12 w-12 rounded-xl bg-cyan-100 dark:bg-cyan-900/30 flex items-center justify-center group-hover:scale-110 transition-transform">
                  ...
                  <span class="text-xs font-medium text-gray-700 dark:text-gray-300">
                    {{ t('new_customers.actions_modal.files', 'Dosyalar') }}
                  </span>
                </button>
                ...
                <NuxtLink 
                  ...
                  class="flex flex-col items-center gap-2 p-4 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-all group"
                >
                  <div class="h-12 w-12 rounded-xl bg-yellow-100 dark:bg-yellow-900/30 flex items-center justify-center group-hover:scale-110 transition-transform">
                    <PencilIcon class="h-6 w-6 text-yellow-600 dark:text-yellow-400" />
                  </div>
                  <span class="text-xs font-medium text-gray-700 dark:text-gray-300">
                    {{ t('new_customers.actions_modal.edit', 'Düzenle') }}
                  </span>
                </NuxtLink>

                <button 
                  v-if="isDeleteable" 
                  @click="confirmDelete(getActiveCustomer)"
                ...
                >
                  ...
                  <span class="text-xs font-medium text-red-600 dark:text-red-400">
                    {{ t('new_customers.actions_modal.delete', 'Sil') }}
                  </span>
                </button>
              </div>
            </div>
            ...
            >
              <div class="p-6 text-center">
                ...
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                  {{ t('new_customers.delete_modal.title', 'Kişiyi Sil') }}
                </h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">
                  {{ tp('new_customers.delete_modal.message', { name: customerToDelete?.name }, '{name} adlı kişiyi silmek istediğinize emin misiniz?') }}
                </p>
                <p class="text-xs text-red-600 dark:text-red-400">
                  {{ t('new_customers.delete_modal.warning', 'Bu işlem geri alınamaz.') }}
                </p>
              </div>
