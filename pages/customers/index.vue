<template>
  <div class="min-h-screen">
      ...
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <div class="flex items-center gap-3">
            <div
              class="p-2.5 bg-gradient-to-br from-red-500 to-red-500 rounded-xl shadow-lg shadow-red-500/25">
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <button v-if="authStore.user?.role === 'admin'" @click="showCreateModal = true"
            class="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-red-400 to-red-500 text-white rounded-xl hover:from-rose-700 hover:to-rose-700 transition-all text-sm font-medium shadow-lg shadow-rose-500/25">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between">
          ...
          <div class="p-3 bg-red-100 dark:bg-red-900/30 rounded-xl">
            <EyeIcon class="h-6 w-6 text-red-600 dark:text-red-400" />
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
      ...
      <div class="p-5">
        <!-- Row 1: Search, Status, Source, User -->
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          ...
          <div v-if="authStore.user?.role === 'admin'">
            ...
            <Combobox v-model="relevantUserFilters" multiple>
              <div class="relative">
                <ComboboxInput
                  class="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-600 rounded-xl text-sm focus:ring-2 focus:ring-rose-500 focus:border-transparent transition-all"
                ...
                <Transition enter-active-class="transition duration-100 ease-out"
                  ...
                  <ComboboxOptions
                    ...
                    <ComboboxOption v-for="user in filteredUsers" :key="user.id" :value="user"
                      ...
                      :class="active ? 'bg-rose-600 text-white' : 'text-gray-900 dark:text-gray-100'">
            ...
            <div v-if="relevantUserFilters.length > 0" class="flex flex-wrap gap-1.5 mt-2">
              <span v-for="user in relevantUserFilters" :key="user.id"
                class="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium bg-rose-100 text-rose-800 dark:bg-rose-900/30 dark:text-rose-300">
                {{ user.name }}
                <button @click.stop="removeUserFilter(user)" class="hover:text-rose-600 dark:hover:text-rose-200">
...
const STORAGE_KEY = 'customerFilters_v3'
...
<style scoped>
.filter-badge {
  @apply inline-flex relative items-center px-3 py-1 rounded-full text-xs font-medium bg-rose-100 text-rose-800 dark:bg-rose-900/20 dark:text-rose-400;
}

.filter-badge button {
  @apply ml-1 text-rose-600 dark:text-rose-400 hover:text-rose-800 dark:hover:text-rose-200 font-bold;
}
</style>