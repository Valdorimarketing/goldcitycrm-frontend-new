  ...
  const fetchCustomers = async (pageOrParams?: number | {
    ...
    try {
      ...
      if (typeof pageOrParams === 'number') {
        ...
        // ✅ Existing filters
        if (pageOrParams.search) queryParams.search = pageOrParams.search
        if (pageOrParams.status !== undefined) queryParams.status = pageOrParams.status
        if (pageOrParams.isActive !== undefined) queryParams.isActive = pageOrParams.isActive
        if (pageOrParams.order) queryParams.order = pageOrParams.order
        if (pageOrParams.relevantUser !== undefined) queryParams.relevantUser = pageOrParams.relevantUser
        if (pageOrParams.statusType) queryParams.statusType = pageOrParams.statusType
        if (pageOrParams.isFirst !== undefined) queryParams.isFirst = pageOrParams.isFirst
        if (pageOrParams.isDoctor !== undefined) queryParams.isDoctor = pageOrParams.isDoctor
        if (pageOrParams.isPricing !== undefined) queryParams.isPricing = pageOrParams.isPricing

        // ✅ NEW: Advanced filters - Bu filtreleri query'e ekliyoruz
        if (pageOrParams.source) queryParams.source = pageOrParams.source
        if (pageOrParams.country) queryParams.country = pageOrParams.country
        if (pageOrParams.relatedTransaction) queryParams.relatedTransaction = pageOrParams.relatedTransaction
        if (pageOrParams.checkupPackage) queryParams.checkupPackage = pageOrParams.checkupPackage
        if (pageOrParams.startDate) queryParams.startDate = pageOrParams.startDate
        if (pageOrParams.endDate) queryParams.endDate = pageOrParams.endDate

        // Add any other custom filter parameters
        const knownParams = [
          'page', 'limit', 'search', 'status', 'isActive', 'order', 
          'relevantUser', 'statusType', 'isFirst', 'isDoctor', 'isPricing',
          'source', 'country', 'relatedTransaction', 'checkupPackage', 
          'startDate', 'endDate'
        ]
